  ...
  return (
    <section id="teams" className="py-12 sm:py-24 relative bg-[#0A0A0B]">
      <div className="max-w-7xl mx-auto px-4 sm:px-8 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 sm:gap-4 border-b border-white/10 pb-12 mb-12">
              ...
              <div key={team.id} className="flex flex-col gap-3">
                ...
                {isSelected && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -12 }}
                    className="sm:hidden rounded-xl border border-white/10 bg-[#0A0A0B] p-4"
                  >
                    <div className="mb-4">
                      <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-yellow-500">Roster</p>
                      <h4 className="text-xl font-black tracking-tighter uppercase italic text-white">{team.name}</h4>
                    </div>

                    {team.roster && team.roster.length > 0 ? (
                      <div className="space-y-3">
                        {team.roster.map(player => (
                          <div key={player.id} className="border border-white/10 bg-white/5 p-3">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <div className="flex items-center gap-2 mb-1">
                                  <h5 className="font-semibold text-sm text-white">{player.name}</h5>
                                  <span className="text-[8px] font-mono uppercase tracking-[0.2em] text-neutral-500">{player.position.slice(0, 3)}</span>
                                </div>
                                <p className="text-xs leading-relaxed text-white/60">{player.bio}</p>
                              </div>
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="text-center py-8 border border-[dashed] border-white/10 bg-white/5">
                        <p className="text-neutral-500 font-mono text-xs uppercase tracking-widest">Roster to be announced</p>
                      </div>
                    )}
                  </motion.div>
                )}
